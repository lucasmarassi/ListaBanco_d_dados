...
documento = colecao.find_one({"mensagem": msg_criptografada.decode()})

if documento:
    msg_descriptografada2 = fernet.decrypt(documento["mensagem"].encode()).decode()
    print(f"Mensagem vinda do MongoDB descriptografada: {msg_descriptografada2}")
else:
